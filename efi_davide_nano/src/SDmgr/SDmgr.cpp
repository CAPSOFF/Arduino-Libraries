...
bool SDmgr_class::begin(){
	...
	if (!INJmgr.battery_status_read() && !bat_check_inhibit()) return false; // return "False" in case battery was OFF, avoiding SD initialization
...
bool SDmgr_class::log_SD_data(){
	...
	SD_writing_buffer[i++] = ADCmgr_meas_binary; // digital inputs status
	...
	if (SD_init_OK == true){ // SD card initialized properly
		...
		if (INJmgr.battery_status_read() || bat_check_inhibit()){ // logs only if Battery is ON (or if battery check inhibit config flag is active)
			...
			if (dataFile){ // log only when the file is open, and when battery is connected (to avoid memory corruption)
				...
				if (INJmgr.battery_status_read() || bat_check_inhibit()) { // Data written only in case battery voltage is present, or if bypass flag is active
