...
extern void fuelino_yield(unsigned long time_now_ms); // Allows Fuelino Yield function to be called
bool SdSpiCard::waitNotBusy(uint16_t timeoutMillis) {
  uint16_t t0 = (uint16_t)(millis() & 0x0000FFFF);
  unsigned long  time_now_tmp;
  do{
	time_now_tmp = millis();
	fuelino_yield(time_now_tmp);
    if (((uint16_t)(time_now_tmp & 0x0000FFFF) - t0) >= timeoutMillis) {
      goto fail;
    }
